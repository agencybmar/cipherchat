...
(() => {
  ...
  const elements = {
    ...
    googleStage: document.getElementById('google-stage'),
    idStage: document.getElementById('id-stage'),
    googleSignInButton: document.getElementById('google-sign-in-button'),
    googleAccountLabel: document.getElementById('google-account-label'),
    authSignOutButton: document.getElementById('auth-sign-out-button'),
  ...
  function getStoredSession() {
    try {
      ...
      if (!value || !isCode(value.code)) return null;
  ...
  function saveSession(session) {
    localStorage.setItem(SESSION_KEY, JSON.stringify({ code: session.code }));
  }
  ...
  function saveContacts() {
    ...
    if (state.code && window.CipherBackend && window.CipherBackend.configured) {
      apiRequest('/api/contacts', {
        method: 'PUT',
        body: { contacts: sanitizeContacts(state.contacts, state.code) }
      }).catch(() => {});
    }
  }
  ...
  async function enterApp(session, sessionInfo) {
    ...
    try {
      const remote = await apiRequest('/api/contacts', { session });
      const merged = new Map();
      for (const contact of sanitizeContacts(remote.contacts || [], state.code)) merged.set(contact.code, contact);
      for (const contact of state.contacts) {
        const current = merged.get(contact.code);
        merged.set(contact.code, current ? {
          ...current,
          ...contact,
          addedAt: Math.min(current.addedAt || Date.now(), contact.addedAt || Date.now()),
          lastActivity: Math.max(current.lastActivity || 0, contact.lastActivity || 0)
        } : contact);
      }
  ...
  function showGoogleStage() {
    elements.googleStage.classList.remove('hidden');
    elements.idStage.classList.add('hidden');
    elements.googleAccountLabel.textContent = '';
  }

  function showIdStage(user) {
    elements.googleStage.classList.add('hidden');
    elements.idStage.classList.remove('hidden');
    const name = user && (user.displayName || user.email);
    elements.googleAccountLabel.textContent = name ? `Signed in as ${name}` : 'Signed in with Google';
    elements.idCode.focus();
  }

  async function openGoogleAccount() {
    if (!window.CipherBackend || !window.CipherBackend.configured) {
      setAuthError('Firebase is not configured yet. Paste your Firebase web app settings into firebase-config.js.');
      return;
    }
    setAuthError('');
    const account = await window.CipherBackend.getAccount();
    if (!account.user) {
    ...
    if (!account.code) {
      localStorage.removeItem(SESSION_KEY);
      showIdStage(account.user);
      return;
    }
    const session = { code: account.code };
    saveSession(session);
    setAuthError('Opening your account...');
  ...
  async function signInGoogle() {
    ...
    setAuthError('Opening Google sign-in...');
    try {
      await window.CipherBackend.signInWithGoogle();
  ...
  async function signOutAccount() {
    try {
      ...
      await window.CipherBackend.signOut();
    } catch (_) {}
    localStorage.removeItem(SESSION_KEY);
  ...
  async function bootstrap() {
    ...
    try {
      ...
      setAuthError(error.message || 'Could not open your CipherChat account.');
      const account = window.CipherBackend && window.CipherBackend.getCurrentUser ? window.CipherBackend.getCurrentUser() : null;
      if (account) showIdStage(account);
      else showGoogleStage();
    }
  }
  ...
  async function registerId(event) {
    ...
    try {
      ...
      showToast(`ID #${code} is now permanently linked to your Google account.`, 'info', 6500);
    } catch (error) {
  ...
  async function checkAvailability() {
    ...
    state.availabilityTimer = window.setTimeout(async () => {
      try {
        const result = await apiRequest(`/api/availability/${code}`);
  ...
  async function downloadBackup() {
    try {
      ...
      showToast('Encryption backup downloaded. Keep it private; it can decrypt your chats on another signed-in device.', 'info', 6500);
    } catch (error) {
  ...
  async function restoreBackup(file) {
    ...
    try {
      ...
      const account = await window.CipherBackend.getAccount();
      if (!account.user) throw new Error('Sign in with Google before restoring an encryption backup.');
      if (account.code !== backup.code) throw new Error(`This backup belongs to ID #${backup.code}, but your Google account owns #${account.code || '----'}.`);
      const session = { code: backup.code };
      const sessionInfo = await apiRequest('/api/session', { method: 'POST', session });
  ...
  function bindEvents() {
    elements.googleSignInButton.addEventListener('click', signInGoogle);
    elements.authSignOutButton.addEventListener('click', signOutAccount);
